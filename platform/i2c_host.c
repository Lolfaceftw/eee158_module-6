...
uint16_t SERCOM2_I2C_Read_Polled(uint8_t addr, uint8_t *buf, uint16_t len) {
    ...
    // Step 2: Wait for and process interrupt flags
    while (!(SERCOM2_REGS->I2CM.SERCOM_INTFLAG & (SERCOM_I2CM_INTFLAG_SB_Msk | SERCOM_I2CM_INTFLAG_ERROR_Msk)));
    ...
    for (pos = 0; pos < len; pos++) {
        // wait for data (skip first iteration as data is already there)
        if (pos > 0) {
            while (!(SERCOM2_REGS->I2CM.SERCOM_INTFLAG & (SERCOM_I2CM_INTFLAG_SB_Msk | SERCOM_I2CM_INTFLAG_ERROR_Msk)));
        }
